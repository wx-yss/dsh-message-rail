...
- **加载反馈**：根据刻度所在 history 分页预估剩余页数；预计超过 1 秒时立即提示，预估较快但实际超时则在 1 秒后补充提示，开始滚动即关闭
...
- 剩余时间只估算历史分页和目标组件准备，不包含后续平滑滚动；当前按固定准备约 0.2 秒、每页约 0.45 秒校准，属于体验预估而非完成时间承诺
...
- 进度：索引记录每条用户消息距最新页的 `historyPage`；结合右侧已加载深度估算剩余页数，预计超过一秒时立即显示，否则实际等待超过一秒再提示；目标组件就绪并开始滚动时立即关闭
