...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS =
      ...
      ".dshmr-tick::before{content:'';position:absolute;left:12px;top:50%;transform:translateY(-50%);width:var(--dshmr-tick-width,6px);height:2px;border-radius:0;background:var(--dshmr-tick-color,var(--dsw-alias-border-l2));transition:width 160ms cubic-bezier(0.2,0.75,0.25,1)}" +
      ...
      ".dshmr-tick:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:-2px;border-radius:0}" +
      ".dshmr-preview{position:fixed;z-index:61;width:min(330px,calc(100vw - 60px));box-sizing:border-box;padding:9px 10px 10px;border:1px solid var(--dsw-alias-border-l1);border-radius:13px;background:var(--dsw-alias-bg-layer-2);box-shadow:0 3px 10px rgba(0,0,0,0.16);opacity:0;transform:translateX(-5px) scale(0.985);transform-origin:left center;pointer-events:none;transition:opacity 110ms ease,transform 150ms cubic-bezier(0.2,0.75,0.25,1),top 130ms ease}" +
      ".dshmr-preview[data-visible='true']{opacity:1;transform:translateX(0) scale(1)}" +
      ".dshmr-preview-user{overflow:hidden;white-space:nowrap;text-overflow:ellipsis;color:var(--dsw-alias-label-primary);font-weight:400;line-height:1.55;font-size:13px}" +
      ".dshmr-preview-answer{display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:4;max-height:calc(1.55em * 4);overflow:hidden;white-space:normal;word-break:break-word;text-overflow:ellipsis;color:#808080;line-height:1.55;font-size:13px;margin-top:4px}" +
    ...
    /** Extract visible answer text from DSH assistant content variants. */
      ...
      return blocks
        .filter(
          (block) =>
            block !== null &&
            typeof block === "object" &&
            (block.kind === "text" || block.type === "text") &&
    ...
    /** Read assistant text from the supported session node payload shapes. */
    function answerTextOfNode(node) {
      const data = node && node.data ? node.data : node;
      if (data === null || typeof data !== "object") return "";
      return assistantTextOf(data.blocks) || assistantTextOf(data.content) || assistantTextOf(data.message && data.message.content);
    }
    ...
    function Rail(props) {
      ...
      /** Ordered user-message marks with the assistant answer until the next user message. */
      const marks = React.useMemo(() => {
        const result = [];
        const orderedKeys = order || [];
        for (let userIndex = 0; userIndex < orderedKeys.length; userIndex += 1) {
          const key = orderedKeys[userIndex];
          const node = nodes && nodes.get(key);
          if (node === undefined || node.kind !== "user") continue;
          const data = node.data || {};
          const answers = [];
          for (let answerIndex = userIndex + 1; answerIndex < orderedKeys.length; answerIndex += 1) {
            const answerNode = nodes && nodes.get(orderedKeys[answerIndex]);
            if (answerNode === undefined) continue;
            if (answerNode.kind === "user") break;
            if (answerNode.kind !== "assistant" && answerNode.kind !== "assistant-step") continue;
            const answerText = answerTextOfNode(answerNode);
      ...
      React.useLayoutEffect(() => {
        ...
        if (readingIndex < 0 || readingIndex >= marks.length) {
          el.scrollTop = el.scrollHeight;
          return;
        }
       ...
       * streams in, the window stays pinned to the bottom (see the rail
       * window layout effect), so the visible tick set is stable and
       * nothing flickers.
       */
      ...
      return React.createElement(
        ...
        hoveredMark !== null &&
          React.createElement(
            ...
            React.createElement("div", { className: "dshmr-preview-user" }, hoveredMark.text),
            React.createElement("div", { className: "dshmr-preview-answer" }, hoveredMark.answer),
          ),
      );
    }
