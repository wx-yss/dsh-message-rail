...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** Re-assert budget for the jump scroll (covers data-change races). */
    const JUMP_SCROLL_ATTEMPTS = 5;
    const JUMP_SCROLL_WAIT_MS = 140;
     ...
     * bounded budget before scrolling. The scroll itself drives the shell's
     * own scrollport (`[data-conversation-scroll]`) directly: a bare
     * scrollIntoView is cancelled by the chat view's bottom-follow state
     * machine while the conversation sits at the bottom (see JUMP_KICK_PX),
     * so the jump kicks the follow bit off the bottom first and re-asserts
     * the scroll until the target row is visible or the viewport has left
     * the bottom band.
     */
    function createJump(sessionsService, sessionId) {
      ...
      return async (key) => {
        ...
        if (sp === null) {
          // DOM contract drift: no shell scrollport — fall back to the native path.
          row.scrollIntoView({ behavior: reducedMotion ? "auto" : "smooth", block: "center" });
          return true;
        }
        /** Centered target scrollTop for the row inside the shell scrollport. */
        const centerTarget = () => {
        ...
        for (let attempt = 0; attempt < JUMP_SCROLL_ATTEMPTS; attempt += 1) {
          ...
          const atBottom = sp.scrollTop + sp.clientHeight >= sp.scrollHeight - 25;
          const r0 = row.getBoundingClientRect();
          const pr0 = sp.getBoundingClientRect();
          const alreadyThere =
            r0.bottom > pr0.top && r0.top < pr0.bottom && Math.abs(sp.scrollTop - target) < 4;
          if (alreadyThere) {
            ok = true;
            break;
          }
          if (atBottom && target < floor - 25 && floor > JUMP_KICK_PX) {
            // Kick: one instant step off the bottom flips the chat view's
            // follow bit on the first scroll event; without it the smooth
            // easing's sub-pixel first frames are misread as "not reader
            // movement" and the shell toBottom()s the jump away.
            sp.scrollTop = floor - JUMP_KICK_PX;
          }
          sp.scrollTo({ top: target, behavior: reducedMotion ? "auto" : "smooth" });
          await delay(JUMP_SCROLL_WAIT_MS);
          const r = row.getBoundingClientRect();
          const pr = sp.getBoundingClientRect();
          const visible = r.bottom > pr.top && r.top < pr.bottom;
          const offBottom = sp.scrollTop + sp.clientHeight < sp.scrollHeight - 25;
          const atTarget = Math.abs(sp.scrollTop - target) < 4;
          if (visible || offBottom || atTarget) {
            ok = true;
            break;
          }
        }
        if (!ok) {
          console.warn("[dsh-message-rail] jump scroll did not settle", {
            key,
            attempts: JUMP_SCROLL_ATTEMPTS,
    ...
    function Rail(props) {
      ...
      /**
       * Two-list rendering: while loading, the rail renders a FROZEN SNAPSHOT
       * of the marks as they were at first paint (the first rendered batch) —
       * nothing moves, nothing drifts, previews stay correct. Internally the
       * full-history load keeps prepending into the real marks. Once loading
       * completes (hasMore=false), display switches to the real marks in one
       * swap, landing the complete rail in a single frame.
       */
      const firstSnapshotRef = React.useRef(null);
      const snapshotLimit = Math.floor(railRect === null ? RAIL_HEIGHT : Math.min(RAIL_HEIGHT, railRect.height) / TICK_STEP);
      if (firstSnapshotRef.current === null && marks.length >= snapshotLimit) {
        firstSnapshotRef.current = marks;
      }
      const displayMarks =
        hasMore === true && firstSnapshotRef.current !== null ? firstSnapshotRef.current : marks;

      const [readingIndex, setReadingIndex] = React.useState(-1);
      /**
       * Black tick policy — two phases, no interplay:
       * - While loading (hasMore=true): the last message of the frozen
       *   snapshot stays black; the conversation is ignored entirely (no
       *   two-way mapping).
       * - Once loading completes (hasMore=false): ONE sync to the reading
       *   position (40% line), then two-way mapping resumes.
       */
      const activeIndex = hasMore === true ? displayMarks.length - 1 : readingIndex;
       ...
       * - While loading: the rail renders the frozen first-paint snapshot,
       *   so the window never needs to move — ticks stay exactly where they
       *   first appeared, and nothing flickers. The conversation is ignored
       *   (no two-way mapping while loading).
       * - Once loading completes: ONE sync to the reading position, then
       *   snap-back driving (below) takes over.
       */
      React.useLayoutEffect(() => {
        const el = railRef.current;
        if (el === null) return;
        if (hasMore === true) {
          // Loading: FREEZE the window exactly where it first rendered.
          // Prepend loads push the content taller above, and any window
          // movement — however faithful to the same messages — reads as
          // the ticks sliding under the cursor. Frozen, the tick lines stay
          // put (their message mapping drifts, invisible during the brief
          // load); the completed-load sync below lands on the reading
          // position once.
          return;
        }
      ...
      }, [displayMarks.length, hasMore, readingIndex]);
       ...
       * visible capacity, or when the whole history is in. While history
       * streams in, the rail shows the frozen first-paint snapshot (see
       * the rail window layout effect), so the visible tick set is stable
       ...
       * real viewport range. While loading the window is frozen, so the
       * preview stays put; after the completed-load swap, scrolling under a
       * stationary cursor hides the preview once the hovered tick leaves the
       * viewport instead of climbing to the top of the screen.
       */
