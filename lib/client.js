...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Rail(props) {
       ...
       * an in-flight loader is never duplicated. Cancellation is owned by a
       * separate session-scoped effect so a hasMore re-run never kills the
       * running loader, and the completion tick re-evaluates the start gate
       * after a stale loader finally settles. */
      const loadAllInFlightRef = React.useRef(null);
      const loadAllDisposedRef = React.useRef(false);
      const loadAllSessionRef = React.useRef(null);
      ...
      }, [sessionId, hasMore, loadAll, loadTick]);

      /** Cancel the loader on unmount or session switch only. */
      React.useEffect(() => {
        const previous = loadAllSessionRef.current;
        loadAllSessionRef.current = sessionId;
        return () => {
          if (loadAllSessionRef.current === sessionId) {
            loadAllSessionRef.current = null;
