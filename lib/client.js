...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const TICK_STEP = 11;
    /** Edge-follow margin: the window scrolls when the black tick gets this close to an edge. */
    ...
    function Rail(props) {
       ...
       * Edge-follow: the black tick moves freely inside the window (natural
       * progress-bar feel); the window only scrolls when the tick reaches
       * the window edge, so it is always visible but never pinned.
       */
      React.useLayoutEffect(() => {
        ...
        if (blackTop < el.scrollTop + EDGE_MARGIN) {
          el.scrollTop = Math.max(0, blackTop - EDGE_MARGIN);
        } else if (blackTop > el.scrollTop + el.clientHeight - EDGE_MARGIN) {
          el.scrollTop = Math.min(maxTop, blackTop - el.clientHeight + EDGE_MARGIN);
        }
