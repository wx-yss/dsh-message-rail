...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * 协调后台分页与用户跳转。加载器只在没有跳转时启动新的一页；新点击
     * 会取代旧点击，旧异步流程不能提前解除新点击持有的暂停状态。
     */
    function createLoadCoordinator() {
      ...
      return {
        ...
        hasActiveLoad: () => activeLoads > 0,
      };
    }

    /**
     * 跳转到指定消息。跳转期间暂停后台分页；若已有分页正在执行，先等待
     * 数据和右侧 DOM 稳定。目标尚未加载时由本跳转流程独占加载，找到后
     * 解除宿主吸底状态并等待滚动真正到位。
     */
    function createJump(sessionsService, sessionId, coordinator) {
      ...
      return async (key) => {
        ...
        try {
          // 已经启动的分页无法取消；等它完成并提交 DOM 后再开始跳转。
          let guard = 0;
          while (guard++ < JUMP_LOOP_CAP) {
            if (!coordinator.isCurrentJump(jumpToken)) return false;
            const snapshot = session.getSnapshot();
            if (!coordinator.hasActiveLoad() && snapshot && snapshot.loadingOlder !== true) break;
    ...
    /** 后台分批加载全部历史；用户跳转期间不启动新分页。 */
    const MAX_PAGES = 400;
    const MAX_STALLS = 5;
    function createLoadAll(sessionsService, sessionId, coordinator) {
      const delay = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
      const captureAnchor = (sp) => {
        const floor = Math.max(0, sp.scrollHeight - sp.clientHeight);
      ...
      const restoreAnchor = async (sp, anchor) => {
      ...
      return async (disposed) => {
        ...
        while (guard < MAX_PAGES) {
          ...
          let after = null;
          const sp = document.querySelector("[data-conversation-scroll]");
          let anchor = sp === null ? null : captureAnchor(sp);
          const updateAnchor = () => {
            if (sp !== null) anchor = captureAnchor(sp);
          };
          if (sp !== null) sp.addEventListener("scroll", updateAnchor, { passive: true });
          try {
            before = snapshot.chat ? snapshot.chat.order.length : 0;
            guard += 1;
            try {
              await session.loadOlder();
            } catch (error) {
            ...
            after = session.getSnapshot();
            const afterCount = after && after.chat ? after.chat.order.length : before;
            if (afterCount > before && sp !== null) await restoreAnchor(sp, anchor);
            if (afterCount === before) {
              // loadOlder 失败时运行时内部吞错（正常 resolve）：窗口无增长
              // 即静默失败，退避重试，连续无进展则放弃。
              stalls += 1;
    ...
    function Rail(props) {
      ...
      /**
       * 双列表渲染：加载期间固定首批刻度，后台继续向真实 marks 前插历史
       * 消息；加载完成后再一次性切换到完整列表。联动始终按消息 key 映射，
       * 不依赖会随着前插而变化的数组下标。
       */
      const firstSnapshotRef = React.useRef(null);
      const snapshotLimit = Math.floor(
        railRect === null ? RAIL_HEIGHT / TICK_STEP : Math.min(RAIL_HEIGHT, railRect.height) / TICK_STEP,
      );
      ...
      const readingIndex = React.useMemo(() => {
        if (readingKey === null) return -1;
        return displayMarks.findIndex((mark) => mark.key === readingKey);
      }, [displayMarks, readingKey]);
      /**
       * 加载期间只在冻结刻度范围内联动；右侧位置超出该范围时保留最后一个
       * 有效刻度，不移动左侧窗口。首次定位完成前默认高亮最新刻度。
       */
      const activeIndex =
        readingIndex >= 0 ? readingIndex : hasMore === true ? displayMarks.length - 1 : -1;
      ...
      /** 加载期间锁定左侧窗口；加载完成后恢复完整列表的跟随与回弹。 */
      React.useLayoutEffect(() => {
        const el = railRef.current;
        if (el === null) return;
        if (hasMore === true) {
          const lockedTop = Math.max(0, el.scrollHeight - el.clientHeight);
          if (Math.abs(el.scrollTop - lockedTop) > 0.5) {
      ...
      }, [displayMarks.length, hasMore, readingIndex, railRect]);
      ...
      /**
       * 跟踪右侧阅读位置。加载期间 keyToIndex 只包含冻结刻度，因此只能
       * 更新首批刻度中的高亮，不会读取后台持续增长的 marks，也不会驱动
       * 左侧窗口滚动。
       */
      React.useEffect(() => {
        if (displayMarks.length === 0) return;
        const keyToIndex = new Map(displayMarks.map((mark, index) => [mark.key, index]));
        const sp = document.querySelector("[data-conversation-scroll]");
        if (sp === null) return;
        const update = () => {
          ...
          if (best >= 0) {
            const lastKey = displayMarks[displayMarks.length - 1].key;
            for (const row of rows) {
              ...
              if (lr.bottom > rect.top && lr.top < rect.bottom) {
                best = displayMarks.length - 1;
              }
              break;
            }
          }
          if (best >= 0) {
            const nextKey = displayMarks[best].key;
            setReadingKey((current) => (current === nextKey ? current : nextKey));
          }
        };
      ...
      }, [displayMarks, sessionId, hasMore]);
        ...
        React.createElement(
          "div",
          {
            ...
            style: {
              ...
              top: railTop,
              height: railHeight,
              overflowY: hasMore === true ? "hidden" : "auto",
              maskImage,
        ...
        ctx.slots.register(
          {
            ...
            inject: (sessionId) => {
              ...
              return {
                ...
                loadAll: createLoadAll(ctx.sessions, sessionId, coordinator),
              };
            },
          },
          Rail,
