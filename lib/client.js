...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Rail(props) {
      ...
      /** Rail fills the scrollport height; ticks center when few, scroll inside when many. */
      const railTop = railRect.top;
      const contentHeight = marks.length * TICK_STEP;
      const padTop = Math.max(0, Math.floor((railRect.height - contentHeight) / 2));
      ...
      if (hoveredMark !== null) {
        ...
        previewTop = Math.max(8, Math.min(window.innerHeight - PREVIEW_HEIGHT - 8, desired));
      }
      ...
      return React.createElement(
        "div",
        {
          ...
          style: { left: railRect.left, top: railTop, height: railRect.height, overflowY: "auto" },
          role: "list",
          "aria-label": "用户消息位置",
          ref: railRef,
          onScroll: () => {
        ...
        React.createElement(
          "div",
          { style: { paddingTop: padTop, display: "flex", flexDirection: "column", alignItems: "flex-start" } },
          marks.map((mark, index) => {
            ...
            return React.createElement("button", {
              ...
              style: { "--dshmr-tick-width": width + "px" },
              role: "listitem",
