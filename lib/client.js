  ...
  factory: (require) => {
    ...
    function createJump(sessionsService, sessionId, coordinator) {
      ...
      const clearFlash = () => {
        ...
        if (flashedRow !== null) flashedRow.classList.remove("dshmr-flash");
        flashTimer = null;
        flashedRow = null;
      };
      const flash = (row) => {
    ...
    function Rail(props) {
      ...
      /** 仅在对话消息流挂载时测量轨道；轨迹页复用滚动容器但不显示刻度。 */
      React.useEffect(() => {
        const el = document.querySelector("[data-conversation-scroll]");
        let chatViewActive = null;
        const measure = () => {
          if (el === null || chatViewActive !== true) return;
        ...
        const syncView = () => {
          const nextActive = el !== null && el.isConnected && el.querySelector("[data-chat-flow]") !== null;
          if (nextActive === chatViewActive) return;
          chatViewActive = nextActive;
          if (nextActive) {
            measure();
            return;
          }
