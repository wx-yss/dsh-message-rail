...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 实测校准：超过一秒才提示，预计耗时包含定位固定开销和逐页开销。 */
    const NOTICE_DELAY_MS = 1000;
    const NOTICE_TICK_MS = 250;
    const ETA_POSITION_MS = 1800;
    const ETA_PAGE_MS = 350;
    ...
    function createJump(sessionsService, sessionId, coordinator) {
      ...
      return async (messageId, onProgress) => {
        ...
        try {
          ...
          report({ phase: "positioning", loadedPages });
    ...
    function Rail(props) {
      ...
      /** 一秒内完成的跳转不提示；较慢跳转显示预计页数、倒计时和实时阶段。 */
      const runJump = (mark) => {
        ...
        const estimatedPages = loaded ? 0 : Math.max(1, targetPage - loadedHistoryPage);
        const startedAt = Date.now();
        let visible = false;
        let latest = {
          phase: loaded ? "positioning" : "loading",
          loadedPages: 0,
        };
        const publish = () => {
      ...
      if (jumpNotice !== null) {
        ...
        const estimatedTotalMs = ETA_POSITION_MS + estimatedPages * ETA_PAGE_MS;
        const remainingSeconds = Math.max(1, Math.ceil((estimatedTotalMs - elapsedMs) / 1000));
        if (jumpNotice.phase === "positioning") {
          noticeDetail =
            (estimatedPages > 0 ? "预计加载 " + estimatedPages + " 页，现已完成 · " : "历史消息已加载 · ") +
            "正在定位 · 预计还需约 " +
            remainingSeconds +
            " 秒";
          noticeProgress = 92;
          ...
          noticeDetail = "正在查找目标消息 · 预计还需约 " + remainingSeconds + " 秒";
          noticeProgress = 18;
        }
      }
